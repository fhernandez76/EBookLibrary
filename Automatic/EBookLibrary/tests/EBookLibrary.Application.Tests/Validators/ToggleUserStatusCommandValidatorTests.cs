...
public class ToggleUserStatusCommandValidatorTests
{
    ...
    {
        var command = new ToggleUserStatusCommand(Guid.NewGuid(), Guid.NewGuid());
    ...
    {
        var command = new ToggleUserStatusCommand(Guid.Empty, Guid.NewGuid());
        var result = _validator.Validate(command);
    ...
    {
        var command = new ToggleUserStatusCommand(Guid.NewGuid(), Guid.Empty);
        var result = _validator.Validate(command);
        result.IsValid.Should().BeFalse();
    ...
    public void Validate_SameUserIdAndRequestingUserId_ShouldFail()
    {
        ...
        var command = new ToggleUserStatusCommand(sameId, sameId);
        var result = _validator.Validate(command);
        result.IsValid.Should().BeFalse();
